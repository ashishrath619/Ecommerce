  ...
  function DisplayAllModel(){
    ...
    const [state, setState] = useState({
      ...
      const fetchData=async()=>{
        var list =await getData('modelnew/displayall')
        setList(list)
    }
    ...
    const fillCategories=()=>{
       ...
       const fetchBrand=async(catid)=>{
           ...
           var list=await postData('brand/displayallbrands',body)
           console.log(list)
           setBrandList(list)
           }
      ...
      if(checkRequire(getDescription)){
        ...
        if(!err){
        var body={category:getCategory,brandid:getBrandId,modelnam:getModel,description:getDescription,modelid:getModelId}
        var result=await postData('modelnew/updateRecord',body)
        if(result){
      ...
      const handleDelete=async(oldData)=>{
        ...
        var result=await postData('modelnew/deleteRecord',body)
      }
      
      const handleClickOpen = async(rowData) => {
        var result=await postData('brand/displayallbrands',{brandid:rowData.brandid})
